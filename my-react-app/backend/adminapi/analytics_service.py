"""P5 數據分析的業務邏輯——聚合計算、驗證規則、Firebase/UsageEvent 查詢，
從 analytics_views.py 抽出來，跟角色檢查／HTTP 請求解析／JsonResponse
包裝這些 HTTP 邊界關注點分開（P4 review BE-16）。這裡的函式全部吃已經
從 request 解析好的純值（date range 字串、tribe 等），不直接碰 Django
Request/Response——parse_date_range() 原本直接吃 `request` 讀
`request.GET`，這裡改成吃三個已經取出的字串，呼叫端（views.py）自己從
request.GET 取值再傳進來，邏輯完全不變，只是換了個取值的位置。
"""
import re
...
def validate_quiz_answer_payload(payload):
    """回傳錯誤訊息字串；合法時回傳 None。"""
...
def parse_date_range(date_range, date_from, date_to):
    """回傳 (start_date, end_date)，含頭尾兩端。date_range：today／7d／30d／
    custom（custom 需要另外帶 date_from/date_to，格式 YYYY-MM-DD）。"""
    ...
    if date_range == "custom":
        try:
            ...
            raise DateRangeError("date_from/date_to 格式錯誤，需為 YYYY-MM-DD")
        if end < start:
            raise DateRangeError("date_to 不能早於 date_from")
        # 沒有跨度上限的話，具 staff 角色的 client 可以要求例如
        # 0001-01-01 到 9999-12-31——即使資料庫沒資料，fill_date_series()
        ...
        if (end - start).days + 1 > _MAX_CUSTOM_DATE_RANGE_DAYS:
            raise DateRangeError(f"自訂日期區間不能超過 {_MAX_CUSTOM_DATE_RANGE_DAYS} 天")
        return start, end
    raise DateRangeError(f"不支援的 date_range：{date_range}")


def fill_date_series(counts_by_date, start_date, end_date):
...
def _join_date_to_local_date(raw):
    ...
    daily_new_registrations()／retention_analytics() 都直接對 UTC
...
def daily_new_registrations(start_date, end_date):
    ...
    return fill_date_series(counts, start_date, end_date)


def build_dashboard_analytics(start_date, end_date, tribe):
    """儀表板真正圖表的聚合，取代 Dashboard.jsx 原本 5 個「即將推出」佔位。
    全部由 UsageEvent 聚合（新註冊例外——那是 Firebase Auth 才有的資料，
    UsageEvent 不知道使用者何時註冊）。回傳完整的回應 dict（含 date_range
    外層），呼叫端（views.py）直接包進 JsonResponse。"""
    ...
    return {
        ...
        "daily_active_users": fill_date_series(daily_active_counts, start_date, end_date),
        "daily_new_registrations": daily_new_registrations(start_date, end_date),
        "tribe_distribution": [
...
def build_search_analytics(start_date, end_date, tribe):
...
def build_quiz_quality_analytics(start_date, end_date, tribe):
    """答對率×鑑別度散佈圖的聚合。鑑別度用古典測驗理論的「高低分組法」
...
def build_retention_analytics():
    ...
    if not cohort_start_by_uid:
        return {"cohorts": [], "max_weeks": _RETENTION_MAX_WEEKS}
    ...
    return {"cohorts": cohorts, "max_weeks": _RETENTION_MAX_WEEKS}
