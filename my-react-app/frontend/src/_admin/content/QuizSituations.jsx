...
export default function QuizSituations() {
    ...
    const load = useCallback(async () => {
        ...
        try {
            ...
            setData(
                await apiGet(
                    `/adminapi/quiz-bank/situations/?${params.toString()}`,
                ),
            );
        } catch (err) {
    ...
    const runAction = async (item, action, body) => {
        ...
        try {
            if (action === 'delete') {
                ...
                await apiDelete(
                    `/adminapi/quiz-bank/situations/${item.id}/`,
                );
            } else {
                await apiPost(
                    `/adminapi/quiz-bank/situations/${item.id}/${action}/`,
                    body,
                );
            }
            await load();
            ...
            if (item.status === 'published') {
                try {
                    const revision = await apiGet(
                        `/adminapi/quiz-bank/situations/${item.id}/pending-revision/`,
                    );
        ...
        try {
            ...
            if (editTarget.id) {
                if (editTarget.status === 'published') {
                    await apiPost(
                        `/adminapi/quiz-bank/situations/${editTarget.id}/pending-revision/`,
                        payload,
                    );
                } else {
                    await apiPatch(
                        `/adminapi/quiz-bank/situations/${editTarget.id}/`,
                        payload,
                    );
                }
            } else {
                await apiPost(
                    '/adminapi/quiz-bank/situations/',
                    payload,
                );
            }
    ...
    return (
        <main className="quiz-bank-admin-page">
            ...
            >
                ...
                <Modal.Body>
                    <Form.Group controlId="quiz-situation-reject-reason">
