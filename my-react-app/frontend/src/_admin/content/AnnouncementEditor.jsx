...
export default function AnnouncementEditor() {
    ...
    const statusEditable = !id || ['draft', 'rejected', 'unpublished'].includes(status);
    ...
    const toggleTribe = (tribe) => update('tribes', form.tribes.includes(tribe) ? form.tribes.filter((value) => value !== tribe) : [...form.tribes, tribe]);
    ...
    const save = async (submitAfterSave) => {
        setError(''); setSuccess('');
        if (!form.title.trim()) { setError('標題為必填'); return; }
        if (form.is_pinned && !form.pin_until) { setError('開啟置頂時，置頂到期日為必填'); return; }
        if (uploading) { setError('請等待封面圖上傳完成'); return; }
        setSaving(true);
        try {
            const saved = id
            ...
            if (!id) navigate(`/admin/content/announcements/${savedId}`, { replace: true });
        } catch (err) { setError(err.message); }
        finally { setSaving(false); }
    };

    if (loading) return <div className="announcement-editor-loading"><Spinner animation="border" /><span>載入公告中…</span></div>;
        ...
        <main className="announcement-admin-page announcement-editor-page">
            ...
            {!editable && (
                <Alert variant="warning">
                    ...
                    送審中的公告請先撤回；已發布的內容請先下架，確保審核者核准的內容與實際發布內容一致。
                    {statusEditable && !CONTENT_EDITORS.includes(role) && '目前角色沒有內容編輯權限，無法在此狀態下修改。'}
                </Alert>
            )}
            {error && <Alert variant="danger">{error}</Alert>}{success && <Alert variant="success">{success}</Alert>}
            <Form className="announcement-editor-card" onSubmit={(event) => { event.preventDefault(); save(false); }}>
                {/* controlId 讓 react-bootstrap 自動幫 Form.Label 跟底下的 Form.Control
                    ...
                    不是只有測試工具找不到而已。 */}
                <Form.Group className="announcement-field" controlId="announcement-title"><Form.Label>標題 <span className="required-mark">*</span></Form.Label><Form.Control required disabled={!editable} value={form.title} onChange={(e) => update('title', e.target.value)} /></Form.Group>
                <Form.Group className="announcement-field" controlId="announcement-body"><Form.Label>內文</Form.Label><Form.Control as="textarea" rows={10} disabled={!editable} value={form.body} onChange={(e) => update('body', e.target.value)} /></Form.Group>
                ...
                <div className="announcement-form-grid">
                    <Form.Group className="announcement-field announcement-switch-field" controlId="announcement-pinned"><Form.Check type="switch" label="置頂公告" disabled={!editable} checked={form.is_pinned} onChange={(e) => update('is_pinned', e.target.checked)} /></Form.Group>
                    {form.is_pinned && <Form.Group className="announcement-field" controlId="announcement-pin-until"><Form.Label>置頂到期日 <span className="required-mark">*</span></Form.Label><Form.Control type="date" required disabled={!editable} value={form.pin_until} onChange={(e) => update('pin_until', e.target.value)} /></Form.Group>}
                </div>
                <div className="announcement-form-grid">
                    <Form.Group className="announcement-field" controlId="announcement-publish-at"><Form.Label>排程發布時間</Form.Label><Form.Control type="datetime-local" disabled={!editable} value={form.publish_at} onChange={(e) => update('publish_at', e.target.value)} /></Form.Group>
                    <Form.Group className="announcement-field" controlId="announcement-unpublish-at"><Form.Label>排程下架時間</Form.Label><Form.Control type="datetime-local" disabled={!editable} value={form.unpublish_at} onChange={(e) => update('unpublish_at', e.target.value)} /></Form.Group>
                </div>
                <div className="announcement-editor-actions"><Button as={Link} variant="outline-secondary" to="/admin/content/announcements">取消</Button>{editable && <><Button type="submit" variant="outline-primary" disabled={saving || uploading}><Save size={17} /> 儲存</Button><Button type="button" variant="success" disabled={saving || uploading} onClick={() => save(true)}><Send size={17} /> 儲存並送審</Button></>}</div>
            </Form>
        </main>
    );
}
